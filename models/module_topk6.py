...
class ModelManager(nn.Module):
    ...
    def __init__(self, args, num_word, num_slot, num_intent):
        ...
        self.__intent_num_lstm = LSTMEncoder(
            self.__args.encoder_hidden_dim + self.__args.attention_output_dim + self.__num_slot,
            self.__args.encoder_hidden_dim + self.__args.attention_output_dim + self.__num_slot,
            self.__args.dropout_rate
        )
        ...
        self.__sentattention_num = UnflatSelfAttention(
            self.__args.encoder_hidden_dim + self.__args.attention_output_dim + self.__num_slot,
            self.__args.dropout_rate
        )
    ...
    def forward(self, text, seq_lens, n_predicts=None):
        ...
        c = self.__sentattention(intent_lstm_out, seq_lens)
        pred_intent__ = self.__intent_decoder(c)
        pred_intent = self.__intent_decoder(intent_lstm_out)
        # print(len(pred_intent__))
        """intent num"""
        intent_num_lstm_out = self.__intent_num_lstm(torch.cat([g_hiddens, pre_slot_lstm_out], dim=-1), seq_lens)
        intent_num_lstm_out = F.dropout(intent_lstm_out, p=self.__args.dropout_rate, training=self.training)
        c2 = self.__sentattention_num(intent_lstm_out, seq_lens)
        # intent_num_lstm_out_mean = intent_num_lstm_out.mean(dim=1)
        pred_intent_number = self.__intent_number_decoder(c)
        """"""
        ...
        # )
        # AGIF式
        intent_index_sum = torch.sum(torch.sigmoid(pred_intent__) > self.__args.threshold, dim=1).unsqueeze(1)
        ...
        # print(intent_index.size())
        """联结同样的复制n次, n为toekn个数"""
        intent = pred_intent__.unsqueeze(1).repeat(1, g_hiddens.size()[1], 1)
        # print(g_hiddens.size(), intent.size())
        slot_lstm_out = self.__slot_lstm(torch.cat([g_hiddens, intent], dim=-1), seq_lens)
...
class UnflatSelfAttention(nn.Module):
    ...
    def __init__(self, d_hid, dropout=0.):
        super().__init__()
        self.scorer = nn.Linear(d_hid, 1)   # 公式3中线性层
        self.dropout = nn.Dropout(dropout)

    def forward(self, inp, lens):       # inp是input; lens是seq_lens列表，如[36, 34, 26, 22, 21, 20, 19, 19, 18, 18, 16, 15, 15, 14, 14, 13]
        batch_size, seq_len, d_feat = inp.size()    # seq_lens是lens的最大值(第一个);  d_feat=256+128
        inp = self.dropout(inp)                     # 不知道dropout起作用没有，inp.shape没有变化

        scores = self.scorer(inp.contiguous().view(-1, d_feat)).view(batch_size, seq_len)   # torch.Size([batch_size, seq_len]) https://zhuanlan.zhihu.com/p/64551412
        max_len = max(lens)
        for i, l in enumerate(lens):    # https://www.runoob.com/python/python-func-enumerate.html
            if l < max_len:
                scores.data[i, l:] = -np.inf
                #print(scores.data[i, l:])

        #print(scores.shape)
        scores = F.softmax(scores, dim=1)   # 公式3中softmax,
        #print(scores)
        """公式4"""
        context = scores.unsqueeze(2).expand_as(inp).mul(inp).sum(1)    # torch.Size([batch_size, 256+128])
        #print(context.shape)
        return context
