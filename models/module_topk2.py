...
class ModelManager(nn.Module):
    ...
    def __init__(self, args, num_word, num_slot, num_intent):
        ...
        self.__intent_num_lstm = BiLSTMEncoder(
            self.__args.encoder_hidden_dim + self.__args.attention_output_dim,
            self.__args.encoder_hidden_dim + self.__args.attention_output_dim,
            self.__args.dropout_rate
        )
    ...
    def forward(self, text, seq_lens, n_predicts=None):
        ...
        intent_num_lstm_out = self.__intent_num_lstm(g_hiddens, seq_lens)
        intent_num_lstm_out = intent_num_lstm_out.mean(dim=1)
        pred_intent_number = self.__intent_number_decoder(intent_num_lstm_out)
        """"""
