...
class TorchDataset(Dataset):
    ...
    def __init__(self, text, slot, intent, intent_number):
        self.__text = text
        self.__slot = slot
        self.__intent = intent
        self.__intent_number = intent_number

    def __getitem__(self, index):
        return self.__text[index], self.__slot[index], self.__intent[index], self.__intent_number[index]

    def __len__(self):
        # Pre-check to avoid bug.
        assert len(self.__text) == len(self.__slot)
        assert len(self.__text) == len(self.__intent)
        assert len(self.__text) == len(self.__intent_number)
...
class DatasetManager(object):
    ...
    def add_file(self, file_path, data_name, if_train_file):
        ...
        intent_number = [len(str(item).split("#")) for item in intent]
        ...
        self.__intent_number_data[data_name] = intent_number
        ...
        if if_train_file:
            ...
            self.__digit_intent_number_data[data_name] = intent_number
    ...
    def batch_delivery(self, data_name, batch_size=None, is_digital=True, shuffle=True):
        ...
        if is_digital:
            ...
            intent_number = self.__digit_intent_number_data[data_name]
        else:
            text = self.__text_word_data[data_name]
            slot = self.__text_slot_data[data_name]
            intent = self.__text_intent_data[data_name]
            intent_number = self.__intent_number_data[data_name]
        dataset = TorchDataset(text, slot, intent, intent_number)
