...
class Processor(object):
    ...
    def __init__(self, dataset, model, args):
        ...
        self.__criterion_num = nn.CrossEntropyLoss()  # Transformer-based Threshold-Free
    ...
    def train(self):
        ...
        for epoch in range(0, self.__dataset.num_epoch):
            ...
            for text_batch, slot_batch, intent_batch, intent_number_batch in tqdm(dataloader, ncols=50):
                ...
                intent_number_var = torch.LongTensor(sorted_intent_number)
                ...
                if self.args.gpu:
                    ...
                    intent_number_var = intent_number_var.cuda()

                random_slot, random_intent = random.random(), random.random()

                pre_slot_out, slot_out, intent_out, intent_number_out = self.__model(text_var, seq_lens)
                ...
                intent_number_loss_alpha = self.args.intent_number_loss_alpha
                slot_loss_alpha = self.args.slot_loss_alpha
                pre_slot_loss_alpha = self.args.pre_slot_loss_alpha
                batch_loss = pre_slot_loss_alpha * pre_slot_loss + slot_loss_alpha * slot_loss + \
                              intent_loss_alpha * intent_loss + intent_number_loss_alpha * intent_number_loss
                ...
                try:
                    ...
                    total_intent_number_loss += intent_number_loss_alpha * intent_number_loss.cpu().item()
                    ...
                    total_intent_number_loss += intent_number_loss_alpha * intent_number_loss.cpu().data.numpy()[0]
            ...
            fitlog.add_loss(total_intent_number_loss, name='intent_number loss', step=epoch)
            fitlog.add_loss(total_pre_slot_loss + total_intent_loss + total_slot_loss + total_intent_number_loss, name='total loss', step=epoch)
            time_con = time.time() - time_start
            print(
                '[Epoch {:2d}]: The  first slot loss on train data is {:2.6f}, last slot loss on train data is {:2.6f}, '\
                'intent data is {:2.6f}, intent number loss is {:2.6f}, cost about {:2.6} seconds.'\
                    .format(epoch, total_pre_slot_loss, total_slot_loss, total_intent_loss, total_intent_number_loss, time_con))
    ...
    def prediction(model, dataset, mode, batch_size, args):
        ...
        all_token = []
        for text_batch, slot_batch, intent_batch, intent_number_batch in tqdm(dataloader, ncols=50):
            padded_text, [sorted_slot, sorted_intent, sorted_intent_number], seq_lens = dataset.add_padding(
                text_batch, [(slot_batch, False), (intent_batch, False), (intent_number_batch, False)],
                digital=False
            )
            ...
            _, slot_idx, intent_idx, intent_number_out = model(var_text, seq_lens, n_predicts=1)
